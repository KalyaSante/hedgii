...
test_rclone() {
    doctor_log "KAWAII" "Testing Cloud Storage..."
    
    if ! command -v rclone >/dev/null 2>&1; then
        doctor_log "FAIL" "rclone not installed"
        return 1
    fi
    ...
    if [[ -n "$remotes" ]]; then
        doctor_log "OK" "rclone remotes configured:"
        ...
        if [[ -n "$first_remote" ]]; then
            ...
            if timeout 10 rclone ls "$first_remote" >/dev/null 2>&1; then
                doctor_log "OK" "Connection successful"
            else
                doctor_log "WARN" "Connection failed or timed out"
                doctor_log "FIX" "Check: rclone config reconnect $first_remote"
            fi
        fi
    else
        doctor_log "FAIL" "No rclone remotes configured"
